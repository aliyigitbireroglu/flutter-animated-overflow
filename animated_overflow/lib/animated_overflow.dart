...
class _AnimatedOverflowState extends State<AnimatedOverflow> {
  ...
  AnimatedOverflowDirection? get _animatedOverflowDirection =>
      widget.animatedOverflowDirection;
  double get _dimensionOfInterest =>
      _animatedOverflowDirection == AnimatedOverflowDirection.VERTICAL
          ? _size.height
          : _size.width;
  ...
  void onPostFrameCallback(Duration duration) {
    if (_lastFrame != null) {
      _frameRate = Duration(
          milliseconds: DateTime.now().difference(_lastFrame!).inMilliseconds);
    }
    _lastFrame = DateTime.now();
    if (!_showSelf) {
      setState(() {
        RenderBox _renderBox = _globalKey!.currentContext!.findRenderObject() as RenderBox;
        _size = _renderBox.size;

        if (_shouldAnimate) {
          _multiTween = MultiTween<DefaultAnimationProperties>()
            ..add(
                DefaultAnimationProperties.offset,
                Tween(begin: 0.0, end: _dimensionOfInterest - _maxDimension!),
                Duration(
                    seconds: (_renderBox.size.width / widget.speed).round()),
                Curves.easeInOut);
        }
  ...
  void initState() {
    ...
    _maxDimension =
        _animatedOverflowDirection == AnimatedOverflowDirection.VERTICAL
            ? widget.maxHeight
            : widget.maxWidth;
    _padding = _animatedOverflowDirection == AnimatedOverflowDirection.VERTICAL
        ? EdgeInsets.only(top: widget.padding, bottom: widget.padding)
  ...
  Widget build(BuildContext context) {
    if (!_showSelf) {
      ...
      return CustomAnimation(
          ...
          builder: (BuildContext context, Widget? myWidget,
              MultiTweenValues<dynamic> animation) {
            try {
              if (_scrollController!.hasClients) {
                _scrollController!.animateTo(
                    animation.get(DefaultAnimationProperties.offset),
                    duration: _frameRate,
                    curve: Curves.linear);
              }
            } catch (_) {}
